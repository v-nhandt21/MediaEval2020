...
import commons
train_info = commons.read_file('../data/split-0/train_balance3_re_dur.tsv')
#train_info = commons.read_file('../data/split-0/balance_multi.tsv')
validation_info = commons.read_file('../data/split-0/autotagging_moodtheme-validation_wn.tsv')
...
val_transform = get_transforms(False, 937)
...
class Model(nn.Module):
    ...
    def __init__(self):
        ...
        self.mv2 = EfficientNet.from_pretrained('efficientnet-b6', advprop=True, num_classes=56)
        #self.mv2_2 = EfficientNet.from_pretrained('efficientnet-b0', num_classes=56)
        #self.mv2 = models.googlenet(pretrained=True)        
...
#criterion = WeightedFocalLoss(gamma=3)
...
val_loader = DataLoader(valid_dataset, 32, shuffle=False, num_workers=0)
train_loader_1 = DataLoader(train_dataset, 32, shuffle=True, drop_last=True, num_workers=4, pin_memory=True)
train_loader_2 = DataLoader(train_dataset, 32, shuffle=True, drop_last=True, num_workers=4, pin_memory=True)
...
#outdir = 'wavenet_feature_onelabel_augment'
outdir = 'wavenet_feature_efficient_highest_b_bs32'
...
np.save('{}/decisions_ex.npy'.format(outdir), decisions)
